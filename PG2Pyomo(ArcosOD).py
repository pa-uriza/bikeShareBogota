...
def balanceo_bicicletas_rule(model,ni,t):
    if t < model.Tmax:
        return ((sum(model.x[no,nd,nj,ni,t] for (no,nd) in model.OD for nj in model.NEstIn[ni] if (no,nd,ni,nj) in model.A ) - sum(model.x[no,nd,ni,nj,t] for (no,nd) in model.OD for nj in model.NEstOut[ni] if (no,nd,ni,nj) in model.A ) + model.z[ni,t])== model.z[ni,(t+1)])
    else:
...
def flujo_salida_inventario_rule(model,ni,t):
    return (sum(model.x[no,nd,ni,nj,t] for nj in model.NEstOut if (no,nd,ni,nj) in model.A) <= model.z[ni,t])
