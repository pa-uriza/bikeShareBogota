...
def balanceo_bicicletas_rule(model,ni,t):
    if t < model.Tmax:
        return ((sum(model.x[od,nj,ni,t] for od in model.OD for nj in model.NEstIn[ni]if (ni,nj) in model.ArcsInOD[od] ) - sum(model.x[od,ni,nj,t] for od in model.OD for nj in model.NEstOut[ni] if (ni,nj) in model.ArcsInOD[od] ) + model.z[ni,t])== model.z[ni,(t+1)])
    else:
...
def flujo_salida_inventario_rule(model,ni,t):
    return (sum(model.x[od,ni,nj,t] for od in model.OD for nj in model.NEstOut if (ni,nj) in model.ArcsInOD[od]) <= model.z[ni,t])
